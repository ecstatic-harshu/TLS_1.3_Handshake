...
from common.relay import (
    ...
    relay_secure_to_http_backend,
    relay_secure_to_secure
)
...
# Wake the accept() loop periodically so
# Ctrl+C / KeyboardInterrupt can be handled
# (blocking accept ignores Ctrl+C on Windows).
ACCEPT_POLL_TIMEOUT = 1.0
...
def secure_connect(
    ...
    try:
        ...
        transcript_display = _display_digest(
            handshake_result.final_transcript_digest
        )
...
def run_secure_server(
    ...
    try:
        ...
        # Allow Ctrl+C to interrupt accept().
...
def handle_gateway_client(
    ...
    """
    PQ-terminate one client, then relay to
    backend.

    backend_pq=True:
        dial backend with secure_connect()
        (quantum-safe proxy ↔ server hop)

    backend_http=True:
        each client message → HTTP POST

    else:
        long-lived raw TCP byte relay
    """
    ...
    try:
        ...
        if backend_pq:
            ...
            logger.info(
                "PQ backend mode — dialing "
                f"{backend_host}:{backend_port} "
                "with secure_connect()"
            )

            backend_session = secure_connect(
                host=backend_host,
                port=backend_port,
                verify_server=False,
            ...
            logger.info(
                "PQ backend session established; "
                "starting PQ↔PQ relay"
            )
...
def run_secure_gateway(
    ...
    Backend-facing leg:
        plain TCP, standard TLS, HTTP POST,
        or a second PQ secure_connect hop
    """
