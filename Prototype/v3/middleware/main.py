...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            "Wrap each client message as HTTP "
            f"POST {BACKEND_HTTP_PATH} "
            "(default: on). Use --no-backend-http "
            "for raw TCP backends."
        )
    )

    parser.add_argument(
        "--backend-http-path",
        default=BACKEND_HTTP_PATH,
        help=(
            "HTTP path for backend POST wrapping "
            f"(default: {BACKEND_HTTP_PATH})"
        )
    )
    ...
    parser.add_argument(
        ...
        default=BACKEND_PQ,
        help=(
            "Protect proxy↔backend with the same "
            "PQ secure_connect handshake "
            f"(default: {BACKEND_PQ}). "
            "Backend must speak this protocol "
            "(e.g. --mode server)."
        )
    )
    ...
    if args.mode == "server":
        ...
        try:
            run_proxy(
                ...
                backend_http_path=
                    args.backend_http_path,
                backend_pq=args.backend_pq
            )
        except KeyboardInterrupt:
            print(
                "\n[MIDDLEWARE] Proxy "
